//
//  BuscaPalavraTableViewController.swift
//  TTDD
//
//  Created by Macintosh HD on 19/11/19.
//  Copyright © 2019 Macintosh HD. All rights reserved.
//

import UIKit

struct DescricaoXmlBusca {
    var id:Int
    var grupo:String
    var kind:String
    var assunto:String
...
class BuscaPalavraTableViewController: UITableViewController, XMLParserDelegate {

    var palavra: String=String()
    var grupo_buscado: String=String()
    var elementName: String = String()
    var descricao:[String]=[]
    var descricoes: [DescricaoXmlBusca]=[]
    var id:  Int=Int()
    ...
    override func viewDidLoad() {
        ...
        print("BuscaPalavra - " + String(palavra))

        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(KindSTableViewController.longPress(sender:)))
        ...
        if sender.state == UIGestureRecognizer.State.began {
            ...
            if tableView.indexPathForRow(at: touchPoint) != nil {
                ...
                if (fase_corrente1=="" && fase_intermediaria1=="" &&
                    ...
                    self.performSegue(withIdentifier: "segueBuscaToTemporal", sender: self)
                }
            }
        }
    }
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "row" {
            let row = DescricaoXmlBusca(id: id, grupo:grupo, kind: kind, assunto:assunto, fase_corrente:fase_corrente, fase_intermediaria: fase_intermediaria,
                                        destinacao:destinacao,observacoes:observacoes,indice:indice,proximo:proximo)
            descricoes.append(row)
            texto = grupo + " " + assunto
            
            //htp://developer.apple.com/documentation/swift/string
            grupo_buscado = eliminar_acentos(sentenca: palavra)
            //var assunto2 = assunto.replacingOccurrences(of: " ", with: "")
            let assunto2 = assunto.uppercased()
            let assunto1 = eliminar_acentos(sentenca: assunto2)

            //int firstIndex = assunto1.indexOf(grupo_buscado)
            //if (!Array.asList(array_mensagem).contains(texto) && (firstIndex>=0 && pos<=50)
            
            if assunto1.contains(grupo_buscado){
                array_proximo.append(proximo)
                array_classes.append(grupo)
                array_assunto.append(assunto)
                texto = grupo + " " + assunto
                array_mensagem.append(texto)
                array_kind.append(kind)
    ...
    func eliminar_acentos(sentenca: String)-> String {
        var result = String()
        //print("antes: "+sentenca)
        /*
        result = sentenca.replacingOccurrences(of: "Á", with: "A")
        result = result.replacingOccurrences(of: "É", with: "E")
        result = result.replacingOccurrences(of: "Í", with: "I")
        result = result.replacingOccurrences(of: "Ó", with: "O")
        result = result.replacingOccurrences(of: "Ú", with: "U")
        result = result.replacingOccurrences(of: "Ç", with: "C")
        result = result.replacingOccurrences(of: "Ê", with: "E")
        result = result.replacingOccurrences(of: "Ô", with: "O")
        result = result.replacingOccurrences(of: "Ã", with: "A")
        result = result.replacingOccurrences(of: "Õ", with: "O")
        result = result.replacingOccurrences(of: "À", with: "A")
        */
        result = sentenca.folding(options: .diacriticInsensitive, locale: .current)
        //print("depois: "+result)
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return array_mensagem.count
    }

    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "celulaReusoBusca", for: indexPath)

        // Configure the cell...
        let selecao1 = array_mensagem[indexPath.row]
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        /*if segue.identifier=="segueBuscaToKindS"{
