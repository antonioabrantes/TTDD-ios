//
//  GPEquiv2TableViewController.swift
//  TTDD
//
...
class GPEquiv2TableViewController: UITableViewController, XMLParserDelegate {

    var elementName: String = String()
    var descricao:[String]=[]
    var descricoes: [DescricaoXmlGP2]=[]
    var id:  Int=Int()
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "row" {
            let row = DescricaoXmlGP2(id: id, symbol1: symbol1, assunto1: assunto1, symbol2: symbol2, assunto2:assunto2)
            descricoes.append(row)
            //print("grupo_buscado"+grupo_buscado)
            if symbol2==grupo_buscado{
                array_classes.append(symbol1)
                array_mensagem.append(assunto1)
                texto = symbol1 + " " + assunto1
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "celulaReusoGP2", for: indexPath)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let alertController = UIAlertController(title: nil, message:
            "Este tela mostra os códigos da Resolução 14/2001 do Conarq (revogada)", preferredStyle: .alert)
