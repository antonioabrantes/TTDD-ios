...
//  TTDD
//
//  Created by Macintosh HD on 15/10/19.
//  Copyright © 2019 Macintosh HD. All rights reserved.
//

import UIKit

struct DescricaoXmlKindG {
    var id:Int
    var grupo:String
    var kind:String
    var assunto:String
...
class KindGTableViewController: UITableViewController, XMLParserDelegate {

    var elementName: String = String()
    var descricao:[String]=[]
    var descricoes: [DescricaoXmlKindG]=[]
    //var elementName: String=String()
    var id:  Int=Int()
    ...
    override func viewDidLoad() {
        ...
        print("KindG - " + String(proxima_tela))
        
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(KindGTableViewController.longPress(sender:)))
        ...
        if sender.state == UIGestureRecognizer.State.began {
            ...
            if tableView.indexPathForRow(at: touchPoint) != nil {
                ...
                if (fase_corrente1=="" && fase_intermediaria1=="" &&
                    ...
                    self.performSegue(withIdentifier: "segueGToTemporal", sender: self)
                }
            }
        }
    }
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "row" {
            let row = DescricaoXmlKindG(id: id, grupo:grupo, kind: kind, assunto: assunto, fase_corrente: fase_corrente, fase_intermediaria: fase_intermediaria,
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "celulaReusoKindG", for: indexPath)
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier=="segueToKindZ"{
            if let indexPath=tableView.indexPathForSelectedRow {
                ...
                //print ("entrei - "+String(proximo1))
            }
        }
        //print("entre no segue")
        if segue.identifier=="segueGToTemporal"{
            //print("entrei no segueGToTemporal "+array_destinacao[1])
            ...
            //print("entrei no if do segueGToTemporal " + iobserva)
