...
class GruposPesquisaTableViewController: UITableViewController, XMLParserDelegate {
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "row" {
            ...
            if grupo==grupo_buscado{
                ...
                */
                pos = pos-1
                if (grupos_detectados==0){
                    for _ in 1...pos+3 { array_kind.append("") }
                    for _ in 1...pos+3 { array_proximo.append(0) }
                    for _ in 1...pos+3 { array_indice.append(0) }
                    for _ in 1...pos+3 { array_classes.append("") }
                    for _ in 1...pos+3 { array_assunto.append("") }
                    for _ in 1...pos+3 { array_texto.append("") }
                    for _ in 1...pos+3 { array_fase_corrente.append("") }
                    for _ in 1...pos+3 { array_fase_intermediaria.append("") }
                    for _ in 1...pos+3 { array_destinacao.append("") }
                    for _ in 1...pos+3 { array_observacoes.append("") }
                }
                grupos_detectados = grupos_detectados + 1

                // normalmente grupos_detectados = 1 pois não há duplicatas, p.ex 026.2
                // mas há exceções p.ex. 049.15, neste caso da primeira vez
                // apenas resete as variáveis acima, na segunda vez teste se
                // já houve um valor prévio carregado, caso positivo então grave
                // na linha pos + 1 o registro atual e deixe em pos o anterior já gravado
                // da primeira vez. Se tiver três grupos repetidos irá funcionar
                // 026.21 

                if array_classes[pos] != "" {
                    if grupos_detectados == 2 {
                        array_kind[pos+1]=kind
                        array_proximo[pos+1]=proximo
                        array_indice[pos+1]=indice
                        array_classes[pos+1]=grupo
                        array_assunto[pos+1]=assunto
                        texto = grupo + " " + assunto
                        array_texto[pos+1]=texto
                        array_fase_corrente[pos+1]=fase_corrente
                        array_fase_intermediaria[pos+1]=fase_intermediaria
                        array_destinacao[pos+1]=destinacao
                        array_observacoes[pos+1]=observacoes
                        count_proximo = count_proximo + 1
                    } else {
                        array_kind[pos+2]=kind
                        array_proximo[pos+2]=proximo
                        array_indice[pos+2]=indice
                        array_classes[pos+2]=grupo
                        array_assunto[pos+2]=assunto
                        texto = grupo + " " + assunto
                        array_texto[pos+2]=texto
                        array_fase_corrente[pos+2]=fase_corrente
                        array_fase_intermediaria[pos+2]=fase_intermediaria
                        array_destinacao[pos+2]=destinacao
                        array_observacoes[pos+2]=observacoes
                        count_proximo = count_proximo + 1
                    }
                } else {
                    array_kind[pos]=kind
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier=="segueGruposToKindS"{
            if let indexPath=tableView.indexPathForSelectedRow {
                ...
                let indice1 = array_proximo[indexPath.row]
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        ...
        if kind1=="c"{
            self.performSegue(withIdentifier: "segueGruposToMain", sender: self)
            return false
        }
        if kind1=="s"{
        ...
        if kind1=="z"{
            self.performSegue(withIdentifier: "segueGruposToKindZ", sender: self)
            return false
        }
        if kind1=="m"{
